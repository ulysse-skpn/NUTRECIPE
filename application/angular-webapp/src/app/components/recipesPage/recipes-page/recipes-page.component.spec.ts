...
import { mockRecipeList } from 'src/app/mocks/itemMock';
...
describe('RecipesPageComponent', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      providers: [ JwtHelperService , RecipesService ],
  ...
  it('should create Recipe Page Component', () => {
  ...
  it('should call sizeRecipeArray method', () => {
    const recipeServiceSpy = spyOn(recipeService,'getSizeArrayRecipes').and.returnValue(of(10))

    spyOn(component,'sizeRecipesArray').and.callThrough()

    component.sizeRecipesArray()
    ...
    expect(component.sizeRecipesArray).toHaveBeenCalled()

    expect(recipeServiceSpy).toHaveBeenCalled()
  })

  it('should call delete method', () => {
    const recipeServiceSpy = spyOn(recipeService,'deleteRecipe').and.returnValue(of([1]))
    spyOn(event,'stopPropagation')
    ...
    recipeService.deleteRecipe(id).subscribe( res => {
      expect(res).toBeDefined()
  ...
  it('should call loadRecipes method', () => {
    spyOn(recipeService,'getAllRecipes').and.returnValue(of(mockRecipeList))

    spyOn(component,'loadRecipes').and.callThrough()
